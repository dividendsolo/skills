...
(GitHub project number + ready Status, or Linear team + project + ready label),
and `autoShip`. If there is no entry, fall back to the GitHub flow for a GitHub
repo and suggest running `/afk-setup` to pin it.

The mechanics differ by tracker (GitHub `gh` commands vs the Linear MCP) but the
discipline is identical. GitHub is described inline below; the **Linear repos**
section at the bottom maps the same steps onto Linear states and MCP calls.

## 1. Pick exactly one ticket
Gather the project's eligible issues. "Open"/ready means the board's ready state
(GitHub `Ready for Agent`; Linear `Todo` + ready label, `unstarted`). Pick in this
priority order, never skipping a step:
...
3. **Ready / Todo** — fresh work, only if (1) and (2) are empty.
...
mine** belong to other agents; do not touch them. Scope filter: if the project
uses a worker label (e.g. `AFK`), only work tickets carrying it.
...
- Closed, or status `Done` (already shipped).
- Status `Future` (parked) — ask if they want to revive it before doing anything.

Otherwise route by status:

| Status (GitHub / Linear) | Path |
|---|---|
| `Triage` / `Needs Info` / `Backlog` | **Interview** — resolve open design questions (see Interview discipline). Resolution flips label + status, NOT code. |
| `Ready for Human` / `Todo`+`HITL` | **Human walkthrough** (see below). You co-pilot; the user does the work. |
| `Ready for Agent` / `Todo`+ready label | **Execute** — sections 4-9. |
...
## Linear repos
When the registry says `tracker: linear`, translate the GitHub mechanics. Load
Linear MCP tools via ToolSearch (`get_issue`, `list_comments`, `save_issue`,
`save_comment`). Fetch by identifier (e.g. `JAM-6`).

| Linear | GitHub equivalent | Path |
|---|---|---|
| `Backlog` | `Triage` / `Needs Info` | Interview; resolution flips to Todo, not code. |
| `Todo` + ready label (e.g. `AFK`), blockers Done | `Ready for Agent` | Execute (4-9). Claim + unassign first. |
| `Todo` + `HITL` label | `Ready for Human` | Human walkthrough. |
| `Todo` with unresolved blockers | blocked | Do not start; name the blocker. |
| `Done` / `Canceled` / `Duplicate` | `Done` | Refuse. |

Kind-of-work labels (bug/enhancement/etc.) may not exist in a young workspace;
infer the route from the body and say so. On GitHub, set status via `gh project
item-edit` and labels via `gh issue edit --add-label`; if a specific CLI invocation
isn't to hand, ask the user once for the exact command rather than doing it manually.
